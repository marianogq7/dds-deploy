        ...
        stage('SonarQube Analysis') {
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'sonarqube_credentials', usernameVariable: 'SONAR_USER', passwordVariable: 'SONAR_PASSWORD')]) {
                    sh 'mvn sonar:sonar -Dsonar.projectKey=libros -Dsonar.host.url=http://sonarqube:9000 -Dsonar.sources=src -Dsonar.exclusions=src/test/** -Dsonar.java.binaries=target/classes -Dsonar.login=sqa_291c1a7eea0fc47bfa53591226a1ca0b80da2595' 
                }
            }
        }
